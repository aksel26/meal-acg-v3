...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch data
    const [statsResult, settingsResult, holidaysResult, mealLogsResult] =
      await Promise.all([
    ...
    if (exportType === "raw") {
      ...
      settingsSheet.addRow({
        setting: "일일 식대 단가",
        value: stats[0]?.daily_allowance || settings?.daily_allowance || 10000,
      });
