...
export async function POST(request: Request) {
  ...
  for (const schedule of schedules) {
    const hasUserId = schedule.user_id !== undefined && schedule.user_id !== null;
    const hasLabel = schedule.label !== undefined && schedule.label !== null && schedule.label.trim() !== "";

    if ((!hasUserId && !hasLabel) || (hasUserId && hasLabel)) {
      return NextResponse.json(
        { error: "Each schedule must have either user_id or label, not both or neither" },
        { status: 400 }
      );
    }
  }
