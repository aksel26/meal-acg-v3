...
export async function GET() {
  try {
    ...
    if (gridData?.rowData) {
      gridData.rowData.forEach((row: any, rowIndex: number) => {
        ...
        row.values?.forEach((cell: any, colIndex: number) => {
          ...
          // 흰색이 아닌 색상이 있는 셀 찾기
          if (backgroundColor && 
              !(backgroundColor.red === 1 && backgroundColor.green === 1 && backgroundColor.blue === 1)) {
    ...
    const groups = values.slice(6).map((group: any, groupIndex: number) => {
      ...
      console.log(`그룹 ${group[0]}: 색칠된 셀 ${coloredCellCount}개, 조정된 person:`, adjustedPersons);
    ...
    console.log(`B7부터 색칠된 셀 ${coloredCells.length}개 발견:`, coloredCells);
