...
const LunchGroupList: React.FC<LunchGroupListProps> = ({ 
  ...
  // 데이터 처리
  const processedTeams = processTeamData(groups);
  
  const isMyTeam = (members: { member: string; isEmpty: boolean }[]) => {
    return userName && members.some(({ member, isEmpty }) => 
      !isEmpty && member.trim().toLowerCase() === userName.trim().toLowerCase()
    );
  };
      ...
      {sortedTeams.map((team) => {
        ...
        return (
          <Card
            ...
            onClick={() => onTeamClick?.(team.rawGroupNumber, team.members.map(m => m.member))}
          >
                ...
                {team.members.map(({ member, isEmpty }, index) => {
                  ...
                  if (isEmpty) {
                    ...
                    return (
                      <div
                        key={`empty-${index}`}
                        className={`flex items-center space-x-2 rounded-lg p-2 text-sm border-2 border-dashed ${
                          isMyTeamHighlight 
