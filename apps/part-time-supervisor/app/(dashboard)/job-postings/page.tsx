    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <span className="text-base text-slate-500">
          총 <span className="font-semibold text-slate-900">{filteredPostings.length}</span>건
          {searchQuery.trim() && jobPostings && filteredPostings.length !== jobPostings.length && (
            <span className="text-xs text-slate-400"> / {jobPostings.length}건</span>
          )}
        </span>
