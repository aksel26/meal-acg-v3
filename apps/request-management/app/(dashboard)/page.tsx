...
import { Briefcase, Building2, CheckCircle2, Inbox, UserRound } from "lucide-react";
...
export default async function DashboardPage() {
  ...
  const stats = await getDashboardStats(user.id, requests);
...
async function getDashboardStats(userId: string, assignedRequests: RequestRecord[]) {
  const now = new Date();
  ...
  return (
    <section>
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <TopCustomerCard stat={stats.topCustomer} />
      </div>
    </section>
  );
}
    ...
    <div className="rounded-xl border border-[#f3f3f3] bg-white px-4 py-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-400">내 큐 활성 요청</p>
        </div>
        <div className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-[#f9f9fa] text-slate-500">
          <Inbox size={20} />
        </div>
