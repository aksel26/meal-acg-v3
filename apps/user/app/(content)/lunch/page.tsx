      ...
      >
        ...
        <div className="px-5 py-4 border-b border-gray-50">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-lg font-semibold text-gray-900">점심조 편성</h1>
              {isLoading ? (
              ...
              <Popover>
                ...
                <PopoverContent className="w-72 p-0" align="end">
                  <div className="p-4 border-b border-gray-100">
                    <h4 className="font-medium text-sm text-gray-900">미추첨 인원</h4>
                    <p className="text-xs text-gray-500 mt-0.5">아직 점심조에 배정되지 않은 인원</p>
                  </div>
                  ...
                  <div className="p-3 max-h-52 overflow-y-auto">
                    {usersLoading ? (
                      ...
                      <p className="text-sm text-gray-500 text-center py-4">모든 인원이 배정됨</p>
                    ) : (
                      <div className="grid grid-cols-2 gap-2">
                        {unassignedMembers.map((member, index) => (
                          <div key={`unassigned-${index}`} className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg">
        ...
        <div className="px-5 py-4">
          <div className="flex gap-8 mb-4">
            <div>
              ...
              <p className="text-sm font-medium text-gray-800">{lunchGroupData?.prevDate || "-"}</p>
            </div>
            <div>
              <p className="text-[11px] text-gray-400 mb-0.5">다음 뽑기</p>
              <p className="text-sm font-medium text-gray-800">{lunchGroupData?.nextDate || "-"}</p>
            </div>
          </div>
      ...
      >
        {isLoading ? (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center">
            <p className="text-sm text-red-500 mb-1">데이터를 불러오는데 실패했습니다</p>
      ...
      <Dialog open={isLotteryOpen} onOpenChange={setIsLotteryOpen}>
        <DialogContent className="max-w-4xl w-[90vw] max-h-[90vh] p-0 rounded-2xl overflow-hidden">
          <DialogHeader className="px-6 py-5 border-b border-gray-100">
            <DialogTitle className="text-base font-semibold text-gray-900">점심조 뽑기</DialogTitle>
            <DialogDescription className="text-sm text-gray-500">뽑기 버튼을 눌러 점심조를 뽑아주세요</DialogDescription>
          </DialogHeader>
