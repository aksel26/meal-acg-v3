...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!employeeName) {
      return NextResponse.json({ success: false, error: "직원 이름이 필요합니다." }, { status: 400 });
    }
    ...
    // 활동비 유형만 필터링
    filteredRows = filteredRows.filter((row) => row[5] && row[5].includes("활동비"));
    ...
    return NextResponse.json({
      success: true,
      data: usageDetails,
      message: `${employeeName}의 ${year}년${month ? ` ${month}월` : ""} 활동비 사용 내역을 조회했습니다.`,
    });
  } catch (error) {
    console.error("활동비 사용 내역 조회 오류:", error);
    return NextResponse.json(
      {
        success: false,
        error: "활동비 사용 내역 조회 중 오류가 발생했습니다.",
