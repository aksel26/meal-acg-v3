...
const processMemberData = (data: string[]): { member: string; isEmpty: boolean }[] => {
  if (!Array.isArray(data)) return [];

  return data.map((item) => {
    const isEmpty = !item || typeof item !== "string" || item.trim().length === 0;
    return {
...
// 조 번호를 파싱하고 정리하는 함수
const parseGroupNumber = (groupNumber: string): { id: number; displayName: string } => {
...
const LunchGroupList: React.FC<LunchGroupListProps> = ({ groups, userName, onTeamClick }) => {
  // 데이터 처리 (memoized)
  ...
  const isMyTeam = useCallback(
    (members: { member: string; isEmpty: boolean }[]) => {
      return userName && members.some(({ member, isEmpty }) => !isEmpty && member.trim().toLowerCase() === userName.trim().toLowerCase());
    },
    [userName]
  );
        ...
        return (
          <Card
            ...
            className={`transition-all duration-200 hover:shadow-md ${
              isMyTeamHighlight ? "bg-blue-50 border border-blue-300 shadow-none" : "bg-white border-none shadow-none"
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <span className={`text-sm ${isMyTeamHighlight ? "text-blue-600 font-medium" : "text-gray-500"}`}>
                    {team.members.filter((m) => !m.isEmpty).length}/{team.members.length}명
                  </span>
            ...
            <CardContent className="pt-0">
              <div className="grid grid-cols-2 gap-2">
                {team.members.map(({ member, isEmpty }, index) => {
                  const isMe = userName && !isEmpty && member.trim().toLowerCase() === userName.trim().toLowerCase();
                  ...
                  if (isEmpty) {
                    ...
                    return (
                      <div
                        key={`empty-${index}`}
                        className={`flex items-center space-x-2 rounded-lg p-2 text-sm border-2 border-dashed ${isMyTeamHighlight ? "border-blue-200 bg-blue-25" : "border-gray-300 bg-gray-25"}`}
                      >
                        <div
                          className={`w-6 h-6 rounded-full border-2 border-dashed flex items-center justify-center text-xs font-medium ${
                            isMyTeamHighlight ? "border-blue-300 text-blue-400" : "border-gray-400 text-gray-400"
                          }`}
                        >
                          ?
                        </div>
                        <span className={`font-medium italic ${isMyTeamHighlight ? "text-blue-400" : "text-gray-400"}`}>빈 자리</span>
                      </div>
                    );
                  }
                  ...
                  return (
                    <div
                      key={`${member}-${index}`}
                      className={`flex items-center space-x-2 rounded-lg p-2 text-sm ${isMe ? "bg-blue-100 border border-blue-200" : isMyTeamHighlight ? "bg-white/70" : "bg-gray-50"}`}
                    >
                      <div
