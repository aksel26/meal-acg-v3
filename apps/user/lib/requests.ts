...
export async function listRequestsForUser(
  ...
  if (view === "mine") {
    ...
    query = query.or(
      `assignee_id.eq.${user.id},assignee_ids.cs.{${user.id}}`,
    );
  }
...
export async function getRequestDetailForUser(id: string, user: SessionUser) {
  ...
  if (!request || !canReadRequest(user, request)) return null;
...
async function getLinkedProjectsForRequest(
  ...
  const projectIds = ((links ?? []) as { project_id: string }[]).map(
  ...
  let query = supabase
    .from("projects")
    .select(
      "id, title, status, due_date, customer_names, created_by, owner_id, manager_ids",
    )
    .in("id", projectIds)
    .order("created_at", { ascending: false });
...
async function getRelatedRequests(
  ...
  const { data, error } = await supabase
    .from("requests")
    .select(
      "id, title, status, priority, created_at, due_date, requester_name",
    )
    .neq("id", current.id)
