-- 2단계 승인 도입 전에 승인된 휴가의 기존 최종 승인 이력을 보존한다.
DROP TRIGGER IF EXISTS trg_prevent_approved_dayoff_approver_change ON public.dayoffs;

UPDATE public.dayoffs
SET final_approver_id = approver_id,
...
CREATE TRIGGER trg_prevent_approved_dayoff_approver_change
  ...
  ON public.dayoffs
  FOR EACH ROW
  EXECUTE FUNCTION public.prevent_approved_dayoff_approver_change();

-- 2단계 승인 후 일반 필드 편집이 1차 담당자를 최종승인자로 덮어쓰지 않게 한다.
CREATE OR REPLACE FUNCTION public.update_dayoff_atomic(
  p_dayoff_id uuid,
  p_editor_id uuid,
  p_is_admin boolean,
  p_changes jsonb
...
BEGIN
  ...
  SET approver_id = CASE
        WHEN p_changes ? 'approverId' THEN v_approver_id
        ELSE approver_id
      END,
      cc_member_ids = v_cc_member_ids
